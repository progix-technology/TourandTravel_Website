          ...
          {featuredTours.map((tour, idx) => {
            ...
            return (
              <div
                ...
                className="group bg-white rounded-[12px] xs:rounded-[14px] sm:rounded-[22px] overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.05)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.1)] transition-all duration-300 flex flex-col justify-between border border-[#13251F]/5"
              >
                {/* Top Image Container */}
                <div className="relative h-[125px] xs:h-[145px] sm:h-[220px] md:h-[250px] w-full overflow-hidden bg-[#071A16]">
                  <img
                    src={tour.image}
                    alt={tour.title}
                  ...
                  <div className="absolute top-1.5 left-1.5 xs:top-2 xs:left-2 sm:top-3.5 sm:left-3.5 px-1.5 sm:px-3 py-0.5 sm:py-1 rounded-[3px] sm:rounded-[4px] bg-[#2E4A35] text-[7px] xs:text-[8px] sm:text-[9.5px] font-bold tracking-wider text-white uppercase shadow-sm truncate max-w-[70%] sm:max-w-none">
                    {tour.category}
                  </div>
                  ...
                  <button
                    ...
                    className={`absolute top-1.5 right-1.5 xs:top-2 xs:right-2 sm:top-3.5 sm:right-3.5 w-5 h-5 xs:w-6 xs:h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-all duration-200 shadow-md ${
                      inWishlist
                        ? 'bg-[#6FCF45] text-[#071A16]'
                        : 'bg-white/90 text-[#13251F] hover:bg-white hover:scale-105'
                    }`}
                  >
                    <Heart className={`w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-4 sm:h-4 ${inWishlist ? 'fill-current' : ''}`} />
                  </button>

                  {/* Bottom-Left Rating Pill */}
                  <div className="absolute bottom-1.5 left-1.5 xs:bottom-2 xs:left-2 sm:bottom-3 sm:left-3.5 flex items-center gap-0.5 sm:gap-1 px-1 xs:px-1.5 sm:px-2.5 py-0.5 rounded-full bg-white text-[#13251F] text-[8px] xs:text-[9px] sm:text-[11px] font-bold shadow-md">
                    <Star className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 fill-[#EAB308] text-[#EAB308]" />
                    <span>{tour.rating.toFixed(1)}</span>
                    <span className="text-[7.5px] xs:text-[8px] sm:text-[10px] text-[#5C6E67] font-normal">
                      ({tour.reviewsCount})
                    </span>
                  </div>
                </div>
                ...
                {/* Content Body */}
                <div className="p-2 xs:p-2.5 sm:p-5 flex-1 flex flex-col justify-between">
                  <div className="flex-1 flex flex-col">
                    {/* Metadata Line */}
                    <div className="flex items-center gap-1 sm:gap-2 text-[8.5px] xs:text-[9px] sm:text-[11px] text-[#5C6E67] mb-0.5 sm:mb-2 font-medium min-h-0 sm:min-h-[18px]">
                    ...
                    <Link to={`/tours/${tour.slug || tour.id}`} className="block">
                      <h3 className="text-xs sm:text-lg lg:text-xl font-bold text-[#13251F] tracking-tight hover:text-[#4F8F45] transition-colors leading-tight font-heading line-clamp-2 min-h-[28px] xs:min-h-[32px] sm:min-h-[48px] flex items-start">
                        {tour.title}
                      </h3>
                    </Link>

                    {/* Subtitle */}
                    <p className="text-[9.5px] xs:text-[10px] sm:text-xs text-[#7A8B84] mt-0.5 sm:mt-1 line-clamp-1 sm:line-clamp-2 min-h-0 sm:min-h-[32px] leading-relaxed font-normal">
                      {tour.subtitle}
                    </p>
                  </div>

                  {/* Footer: Price + Book Now Button */}
                  <div className="mt-1.5 sm:mt-auto pt-1.5 sm:pt-4 border-t border-[#13251F]/10 flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2">
                    {/* Price */}
                    <div className="flex flex-col">
                      <span className="text-[7px] xs:text-[7.5px] sm:text-[9px] uppercase font-bold tracking-wider text-[#7A8B84] block leading-tight">
                        STARTING FROM
                      </span>
                      <div className="text-xs sm:text-base lg:text-lg font-bold text-[#13251F] font-heading leading-tight mt-0.5">
                        ₹{tour.startingPrice.toLocaleString('en-IN')}
                      </div>
                      <span className="text-[7.5px] xs:text-[8px] sm:text-[10.5px] font-normal text-[#7A8B84] leading-tight block whitespace-nowrap">
                        / person
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2.5">
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto bg-[#2E4A35] text-white px-2 sm:px-3.5 py-1.5 rounded-[5px] sm:rounded-[6px] text-[8.5px] xs:text-[9px] sm:text-[10.5px] font-bold uppercase tracking-wider hover:bg-[#233A29] transition-colors shadow-sm active:scale-95 text-center"
                      >
                        BOOK NOW
                      </button>
                    </div>
